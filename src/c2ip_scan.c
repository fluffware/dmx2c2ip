...
gboolean
handle_packet(guint packet_type, guint packet_length,
	      const guint8 *buffer, GError **err)
{
  ...
  case C2IP_DEVICE_BASE_STATION:
    g_debug("Found base station");
    break;
  case C2IP_DEVICE_CAMERA_HEAD:
    g_debug("Found camera");
    break;
  ...
  {
    gchar *dev_addr_str = g_inet_address_to_string(dev_addr);
    g_debug("Device: '%s' %s:%d", name, dev_addr_str, dev_port);
    g_free(dev_addr_str);
  }
  g_free(name);
		       ...
		       gpointer user_data)
{
  ...
  case G_IO_IN:
	...
	if (packet_length + 4 != size) {
	  ...
	  GError *err = NULL;
	  if (!handle_packet(protocol_type, packet_length, &buffer[4], &err)) {
